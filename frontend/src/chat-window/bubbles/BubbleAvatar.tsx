...
const useStyles = makeStyles((theme) => {
  return {
    container: {
      ...
      marginRight: (p: { isUser: boolean }) =>
        p.isUser ? "" : theme.spacing(0.5),
      marginLeft: (p: { isUser: boolean }) =>
        !p.isUser ? "" : theme.spacing(0.5),
    },
    avatar: {
      background: "white",
      ...
      padding: "3px",
      width: "40px",
      borderRadius: (p: { isUser: boolean }) =>
        `${p.isUser ? "50% 50% 50% 0" : "50% 50% 0 50%"}`,
      transformOrigin: (p: { isUser: boolean }) =>
        `${p.isUser ? "bottom left" : "bottom right"}`,
    },
  };
});

export const BubbleAvatar: React.FC<{ params: ChatParams; entry: ChatEntry }> =
  ({ params, entry }) => {
    const isUser = entry.isOwn;
    const avatar = isUser
      ? params.ownAvatar || defaultUserAvatar
      : params.otherAvatar || defaultOtherAvatar;
