...
import {ComponentProperty, BubbleParams, CocoResponse, CoCoChatWindowParams} from "./types";
...
export const CoCoChatWindow = (p: CoCoChatWindowParams) => {
    ...
    const [botGreeting, setBotGreeting] = useState(p.botGreeting || "Type anything to get started!");
    useEffect(() => {
    ...
    const isBotDoneTyping = useBotTyping(content, setContent, lastInputValue || botGreeting);

    const [serverReply, setServerReply] = useServerReply(componentId, inputParams || [], lastInputValue);
    ...
    useEffect(() => {
        ...
        chatState.params = [
            ...
            ...Object.keys(lastContext).reduce((acc, cur) => {
                acc.push({name: cur, value: lastContext[cur], id: uuid(), messageId: lastEntry.id});
                return acc;
            }, [] as BubbleParams[])
        ];
        chatState.rawRepliesData.push({messageId: lastEntry.id, data: lastResultData});
    ...
    return (
        <>
            <ChatWindow
                {...{
                    title: componentName,
                    headerAdditionalContent: <HeaderControl {...{state: chatState}} />,
            ...
            />
            <ReplyDetailsDialog {...{data: replyDetails, onClose: () => setReplyDetails(undefined)}} />
        </>
    );
};
